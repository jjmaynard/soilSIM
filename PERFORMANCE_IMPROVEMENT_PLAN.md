...
([`fuse_texture_group_batch()`](https://jjmaynard.github.io/soilSIM/reference/fuse_texture_group_batch.md)/`_batch_core()`),
with the [`rnorm()`](https://rdrr.io/r/stats/Normal.html) stream ordered
to match the original per-cell consumption order exactly (bit-identical
output, not just statistically similar). **Real bug found and fixed
along the way**: the first version processed a whole
...
original scalar loop inline and asserts
[`fuse_texture_group_batch()`](https://jjmaynard.github.io/soilSIM/reference/fuse_texture_group_batch.md)
