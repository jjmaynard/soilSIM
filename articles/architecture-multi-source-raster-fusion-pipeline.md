...
internal `fuse_lognormal_adaptive()`, which — below `threshold_cells` —
forces
...
`"metalog"` goes to the internal `fuse_metalog_adapter()`, which fits
both sides via
...
**Behavior**: Snaps the depth window via
...
three times (once each for `output_type` `"prediction"`,
`"95% low prediction interval"`, `"95% high prediction interval"`, each
wrapped in `tryCatch` with a warning on failure), extracting the layer
named `paste0(solus_variable, "_", depth_slice, "_cm_", suffix)`
(suffixes `p`/`l`/`h` respectively) from each result. This exact
output-layer-naming assumption was verified against a live,
network-connected `fetchSOLUS()` call before porting, not assumed from
