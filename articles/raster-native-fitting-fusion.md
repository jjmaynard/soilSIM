...
empirical_df$source <- factor(empirical_df$source,
  levels = c("Prior (SSURGO)", "Likelihood (SOLUS)", "Posterior (fused)"))

ggplot(empirical_df, aes(x = x, y = density, color = source)) +
  geom_line(linewidth = 0.8) +
  facet_wrap(~pixel, scales = "free_y") +
  scale_color_viridis_d(name = NULL) +
  labs(title = "Prior, likelihood, and posterior at four individual pixels",
       subtitle = "Reconstructed from each source's own real percentiles (metalog, linear_cdf fallback)",
       x = "Clay content (%)", y = "Density") +
  theme_minimal() +
...
These reconstructed curves can be visibly skewed - a real metalog fit
through asymmetric percentiles, unlike
...
posterior), unlike a Gaussian, which always trails off smoothly toward
$`\pm\infty`$ regardless of whether the real data actually supports that
tail. Any pixel listed by the fallback check above is a case where even
the more flexible metalog couldn’t stay monotonic through that source’s
percentiles, and fell back to the plain interpolated shape instead -
still real percentile data, just piecewise-linear-in-probability rather
than smooth.
