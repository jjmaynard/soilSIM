...
**Purpose**: Accepts either a flat `simulation_config` (e.g. `list(distribution_type = "normal")`, the shape shown in `simulate_monte_carlo()`'s own `@examples`) or an already-nested one (`list(monte_carlo = list(...))`), and normalizes to the nested shape `merge_configurations()` expects.
**Signature**:
```r
...
```
**Parameters**: `properties` - possibly including pseudo-properties; `composition_plan` - result of `core-distributions.R`'s `resolve_composition_groups()`, or `NULL` (pass-through).
...
**Behavior**: For "ordinary" (non-composition-group) properties, calls `extract_property_parameters()` per horizon per property. For each *active* composition group's pseudo-properties (e.g. `ilr1`/`ilr2`), both are fit together, once per horizon, via `core-distributions.R`'s `estimate_ilr_moments_mc()` from the group's real `_l/_r/_h` triplets (their joint covariance only makes sense computed jointly). Only the marginal SDs from that MC estimate are kept (`family = "normal"`, `source = "ilr_mc"`); the ilr1<->ilr2 *off-diagonal* is deliberately discarded because the later cross-horizon empirical correlation step (`estimate_property_correlations()`) owns that correlation instead - avoiding double-counting per-observation uncertainty propagation against cross-sample empirical correlation. Finishes by logging a per-property coverage summary via `summarize_parameter_extraction()`.
...
- `fuse_one_property_prior(prior, likelihood, is_vector_likelihood, n_samples, supported_closed_form, prop, horizon_index)` - implements the per-property closed-form/general dispatch described above, handling `normal`, `lognormal` (converts the raw-space likelihood to log-space via `convert_normal_to_lognormal()` before fusing, since the lognormal prior fit is already stored in log-space), and `beta` (rescales a raw-scale likelihood mean/var onto the prior's own `[lower, upper]` support before fitting moments, since `fuse_beta()`'s alpha/beta addition requires shared support; falls back to a Normal-moments round-trip via `fuse_normal_normal()` if the direct beta fusion is infeasible).
...
**From `core-distributions.R`** (percentile-triplet fitting, ILR compositional machinery, correlation-matrix utilities):
...
`fuse_distribution()`, `fuse_property()`, `fuse_texture_group_from_triplets()`, `fuse_normal_normal()`, `convert_normal_to_lognormal()`, `fuse_beta()`, `convert_moments_to_beta()`, `convert_beta_to_moments()`.
...
- **GP depth modeling & multivariate adjustment** (`core-gp.R`, `core-gp.R`) - call `simulate_monte_carlo()` directly per cokey (choosing a single flat correlation matrix via `select_simulation_correlation_matrix()` from a genhz-keyed set), then flatten the resulting `[horizon, property, realization]` array to long format (`result$simulation_data`) for depth-trend and NRCS/local GP adjustment.
