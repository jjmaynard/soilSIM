...
- **Behavior**: Exposes `query_osd_distinctness()`'s already-computed,
  ...
  `build_depth_correlation_kernel()`) - previously `bound_sd` was computed
  and consumed only inside `simulate_and_perturb_soil_profiles()`, as
  `aqp::perturb()`'s `boundary.attr` for horizon-*depth* perturbation, and
  never reached the property-*value* Monte Carlo simulation pipeline. Calls
