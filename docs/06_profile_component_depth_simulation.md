...
- **Behavior**: Groups by `cokey`; within each group, drops rows whose
  ...
  cumulative sums of the horizon thicknesses - preserving each horizon's
  thickness while removing the organic layer's contribution to depth.
...
- **Returns**: A data frame stacking simulated property values across all
  ...
  attached - plus `bound_sd`, whenever `sim_cokey` itself has a `bound_sd`
  column (see `attach_osd_boundary_distinctness()`), broadcast unchanged onto
  every realization for that horizon row; absent entirely for any caller
  whose `sim_cokey` never had one.
...
- **Parameters**: `hz_data` - a data frame with at least `compname`, `hzname`,
  and `genhz` columns (the same shape `simulate_ssurgo_mapunit_draws()`
  builds right after `classify_genhz()`).
- **Returns**: `hz_data` with a `bound_sd` numeric column added/overwritten -
  one value per `compname`/`genhz` combination, `NA` for any combination the
  OSD lookup couldn't resolve. On OSD lookup failure (e.g. no network
  access), degrades to an all-`NA` `bound_sd` column rather than erroring.
- **Behavior**: Exposes `query_osd_distinctness()`'s already-computed,
  already-genhz-averaged `bound_sd` value on arbitrary horizon data, so it
  can also gate the joint-copula vertical-correlation depth kernel (see the
  GP Depth Modeling & Multivariate Adjustment module,
  `build_depth_correlation_kernel()`). `bound_sd` is also used by
  `simulate_and_perturb_soil_profiles()` as `aqp::perturb()`'s `boundary.attr`
  for horizon-*depth* perturbation. Calls
  `query_osd_distinctness()`, then reuses the identical
  ...
  and left-joins the result onto `hz_data`. **Joins case-insensitively**
  (`toupper()` on both sides): `soilDB::fetchOSD()` (called inside
  `query_osd_distinctness()`) returns its `id` column upper-cased regardless
...
- A `mukey` (string/numeric) - sole input to
  `simulate_profile_depths()`, which internally queries SSURGO.
...
- **`simulate_and_perturb_soil_profiles()`** - requires `soil_profile`'s
  ...
  this function *directly* (not via `simulate_profile_depths()`)
  ...
  unchanged.
- ~~**`simulate_profile_depths()`** - has the same underlying
