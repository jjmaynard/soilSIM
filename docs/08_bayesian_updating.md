...
### Integration status
The functions in this file are standalone building blocks; the tabular Monte Carlo pipeline in `monte-carlo.R` does not call them directly.

`R/raster-fusion.R` uses these same functions (`bayes_update_normal_normal()`, `fuse_beta()`, `fuse_gamma()`, the `*_to_moments()`/`moments_to_*()` helpers, `normal_to_lognormal_params()`/`lognormal_to_normal_params()`, `bayesian_update()`, and `fuse_bivariate_normal()`) as the computational core of an adaptive, per-cell fusion pipeline (`fuse_adaptive()`/`fuse_property_adaptive()`) that routes between the closed-form and general tiers by AOI cell count and includes Lognormal/metalog adapters and infeasible-cell fallback handling. In other words: `bayesian-updating.R` supplies the scalar primitives, and `raster-fusion.R` assembles the raster-native equivalents into a complete pipeline.
