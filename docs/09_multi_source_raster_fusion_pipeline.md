...
`simulate_from_percentiles(method = "linear_cdf")`, fuses via `update_prior()`, and
...
`fuse_normal_normal()`. Any other `dist` value `stop()`s. The returned list always carries
...
`fetch_solus_percentiles_multiproperty()` call **per window** (covering every variable any standalone
...
structure is preserved. The multi-property / multi-window analogue of `lookup_mukey_draws()`. Takes
...
- `method = "saxton_rawls"` (**default**): `fetchSOLUS()` publishes no water-retention variable, so
  ...
  vectorized port of `compute_saxton_rawls()`) derives field capacity / wilting point per
...
The vectorized `terra`-native counterpart of `compute_saxton_rawls()` (`R/adapter-ssurgo-infill.R`)
...
| Pedotransfer-function error | **Not propagated.** Saxton-Rawls is applied deterministically per realization; the +/-15% `_l`/`_h` band `compute_saxton_rawls()` returns is not used. |
...
- `R/adapter-ssurgo-infill.R` — the per-pixel bridge's `saxton_rawls_raster()` is a `terra`-native port of
  this file's `compute_saxton_rawls()` (the equations are duplicated, not called, because
