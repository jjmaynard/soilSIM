...
- **Get Started**:
  [`vignette("getting-started-monte-carlo")`](https://jjmaynard.github.io/soilSIM/articles/getting-started-monte-carlo.md)
  walks through the full tabular pipeline (acquisition -\> infilling -\>
  Monte Carlo -\> statistics -\> GP depth trends) against real SSURGO
  data. Further vignettes cover profile and depth simulation, available
  water storage via Van Genuchten/ROSETTA, multi-source raster fusion
  (SSURGO x SOLUS100), per-pixel fused ensembles, and
  function-by-function tours of each subsystem.
...
MIT (c) Jonathan Maynard - see
[LICENSE.md](https://jjmaynard.github.io/soilSIM/LICENSE.md).
