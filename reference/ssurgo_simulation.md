...
in `R/core-simulation.R`), aggregates to a requested depth window, and
rasterizes per-mukey percentiles for
...
(`R/adapter-ssurgo-acquire.R`) default `properties` and always-included
base columns (`comppct_l/r/h`, all `_l/_r/_h` horizon triplets) already
match
[`simulate_component_composition()`](https://jjmaynard.github.io/soilSIM/reference/simulate_component_composition.md)/[`simulate_cokey_generalized()`](https://jjmaynard.github.io/soilSIM/reference/simulate_cokey_generalized.md)'s
