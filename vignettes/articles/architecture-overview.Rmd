...
| -- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| 01 | [Data Acquisition & Processing](architecture-data-acquisition-processing.html)                 | SSURGO data acquisition, cleaning, infilling                    | `ssurgo-acquisition.R`, `ssurgo-processing.R`, `data-infilling.R`                                                     |
| 02 | [Statistics & Diagnostics](architecture-statistics-diagnostics.html)                           | Statistical characterization + workflow QA                      | `statistics.R`, `validation-diagnostics.R`                                                                              |
| 03 | [Distribution Fitting & Correlation Matrices](architecture-distribution-fitting-correlations.html)     | Percentile fitting, ILR transforms, correlation matrices (core) | `distributions.R`, `percentile-sampling.R`, `kssl-reference-correlations.R`                                           |
| 04 | [Monte Carlo Simulation](architecture-monte-carlo-simulation.html)                           | Correlated Monte Carlo simulation engine (core)                 | `monte-carlo.R`                                                                                                           |
| 05 | [GP Depth Modeling & Multivariate Adjustment](architecture-gp-modeling-multivariate-adjustment.html) | GP depth-trend modeling + MC/GP integration (core)              | `gp-modeling.R`, `multivariate-adjustment.R`                                                                            |
| 06 | [Profile, Component & Depth Simulation](architecture-profile-component-depth-simulation.html)   | Component composition + horizon depth/thickness simulation      | `property-simulation.R`, `depth-simulation.R`                                                                           |
| 07 | [AWS / Van Genuchten Modeling](architecture-aws-van-genuchten-modeling.html)                   | Available water storage / Van Genuchten modeling                | `aws-simulation.R`                                                                                                        |
| 08 | [Bayesian Updating (Scalar)](architecture-bayesian-updating.html)                     | Scalar Bayesian updating/fusion (core)                          | `bayesian-updating.R`                                                                                                     |
| 09 | [Multi-Source Raster Fusion Pipeline](architecture-multi-source-raster-fusion-pipeline.html) | Raster-native fusion core + SSURGO/SOLUS100 adapters            | `distribution-fitting-raster.R`, `raster-fusion.R`, `raster-cache.R`, `ssurgo-simulation.R`, `solus-simulation.R` |
...
- **Fixed**: `validate_monte_carlo_quality()` (`validation-diagnostics.R`, see
  [Statistics & Diagnostics](architecture-statistics-diagnostics.html)) called `generate_simulation_diagnostics()` with 2 arguments, but
...
- **Fixed**: `raster-fusion.R`'s top-of-file comment used to say `run_stage1_fusion()`/
  ...
  [Multi-Source Raster Fusion Pipeline](architecture-multi-source-raster-fusion-pipeline.html)).
- **Fixed**: the `sim_comppct` integration gap between `property-simulation.R` and
  `depth-simulation.R` (see [Profile, Component & Depth Simulation](architecture-profile-component-depth-simulation.html)) - `simulate_profile_depths_by_mukey()`
...
- **Not a bug, on closer inspection**: `utils.R`'s `get_predefined_properties("ssurgo")` calling
  ...
  worth knowing about (see [Utilities](architecture-utilities.html)). Left as-is, along with `utils.R`'s few unused exported
