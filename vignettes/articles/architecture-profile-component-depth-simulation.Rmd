...
- **Behavior**: Exposes `query_osd_distinctness()`'s already-computed,
  ...
  [GP Depth Modeling & Multivariate Adjustment](architecture-gp-modeling-multivariate-adjustment.html)
  module, `build_depth_correlation_kernel()`) - previously `bound_sd` was
  computed and consumed only inside `simulate_and_perturb_soil_profiles()`,
  as `aqp::perturb()`'s `boundary.attr` for horizon-*depth* perturbation, and
